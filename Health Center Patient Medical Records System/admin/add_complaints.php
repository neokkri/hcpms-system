<?php
	if(ISSET($_POST['save_complaints'])){
		...
		if(($section == "Prenatal" || $section == "Maternity") && ($f['gender'] == "Male")){
				echo "<script>alert('Invalid SECTION!')</script>"; // males not allowed in prenatal / maternity section
			}
			else{
			$conn = new mysqli("localhost", "root", "", "hcpms") or die(mysqli_error());
			$conn->query("INSERT INTO `complaints` (date, complaints, remark, itr_no, section, status) VALUES('$date', '$complaints', '$remarks', '$itr_no', '$section', 'Pending')") or die(mysqli_error());
			$conn->close();
			}
	}	