<!DOCTYPE html>
<?php
	require_once 'logincheck.php'; // requires admin's session
?>
<html lang = "eng">
	<head>
	...
	<div class = "navbar navbar-default navbar-fixed-top" style="background-color: #005c99">
		<label class = "navbar-brand">Health Center Patient Record Management System </label>
			<?php
				...
				$q = $conn->query("SELECT * FROM `admin` WHERE `admin_id` = '$_SESSION[admin_id]'") or die(mysqli_error()); //retrieves admins from database
				$f = $q->fetch_array();
			?>
	...
	<div id = "content">
		...
		<div id = "add" class = "panel panel-success">	
			<div class = "panel-heading">
				<label>ADD ADMINISTRATOR</label> <!-- adding administrator -->
		...
		<div class = "panel panel-primary">
			<div class = "panel-heading">
				<label>ACCOUNTS / ADMINISTRATOR</Label> <!-- displays admin accounts created -->
			</div>
			<div class = "panel-body">
				...
				<table id = "table" class = "display" cellspacing = "0"  >
					...
					<tbody>
					<?php
						$conn = new mysqli("localhost", "root", "", "hcpms") or die(mysqli_error()); // retrieves admins from database
...
<script type = "text/javascript">
	function confirmationDelete(anchor)
		{
			var conf = confirm('Are you sure want to delete this record?'); // delete confirmation
			if(conf)
